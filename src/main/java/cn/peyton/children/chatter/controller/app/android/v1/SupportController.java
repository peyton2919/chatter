...
public class SupportController extends AppController {
	...
	public JSONResult<SupportParam> create(SupportParam supportParam, HttpServletRequest request) {
		...
		verifyTokenAndVoluation(request);
		SupportParam _param = supportService.isSupport(baseUserParam.getId(), supportParam.getPostId());
		//判断当前记录是否有操作过
		if (null != _param) {
		...
		supportParam.setUserId(baseUserParam.getId());
